...
class AdvancedTapeDER(TripletModel):
    K_stretch: jax.Array
    D: jax.Array           
    kappa_0: jax.Array     
    nu: jax.Array          
    ...
    def __call__(self, strains: jax.Array) -> jax.Array:
        ...
        active_kappa_crit = jnp.where(bend_x > 0, self.kappa_crit_pos, self.kappa_crit_neg)
        
        # 4. Bending Energy
        flattening = jax.nn.sigmoid((jnp.abs(bend_x) - jnp.abs(active_kappa_crit)) / self.eps)
...
if __name__ == "__main__":
    ...
    print(f"Running Active Learning Comparison: {args.dataset} for {args.seeds}")

    output_loss = {}
    for method in tqdm.tqdm(ActiveMethod, unit="method"):
        valid_losses = []
        config = TrainConfig(
            ...
            N_epochs_per_addition=500,
            N_additions=9,
            N=5,
        )
        seed_pbar = tqdm.tqdm(range(args.seeds), desc=method.name, unit="seeds")
        for i in seed_pbar:
            _, valid_loss = train(
            ...
            seed_pbar.set_postfix(
                {"Valid Loss": f"{np.median(np.asarray(valid_losses), axis=0)[-1]:.4f}"}
            )
        output_loss[method.name] = np.asarray(valid_losses)
    np.savez(f"active_{args.dataset}_{args.seeds}.npz", **output_loss)
