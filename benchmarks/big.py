...
if __name__ == "__main__":
    ...
    parser.add_argument("--samples", type=list[int], default=[10, 128, 256, 512])
    args = parser.parse_args()
    dataset = datasets[args.dataset]

    output_loss = {}
    for m in tqdm.tqdm([IniMethod.MAX_S, IniMethod.RANDOM], desc="Methods"):
        method_results = []  # To store [num_samples, num_seeds]
        for n in tqdm.tqdm(args.samples, desc="Samples"):
            seed_losses = []
            config = TrainConfig(
            ...
            seed_pbar = tqdm.trange(args.seeds, desc=f"N={n}", leave=False)
            for i in seed_pbar:
                _, last_valid_loss = train(
                ...
                seed_losses.append(last_valid_loss)
                seed_pbar.set_postfix({"Loss": f"{np.nanmedian(seed_losses):.4e}"})

            method_results.append(seed_losses)

        output_loss[m.name] = np.array(method_results)

    # Use keyword expansion to save correctly
    np.savez(f"big_{args.dataset}.npz", **output_loss)
